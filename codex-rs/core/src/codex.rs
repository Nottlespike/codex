...
impl Session {
    ...
    async fn record_initial_history(&self, conversation_history: InitialHistory) {
        ...
        match conversation_history {
            InitialHistory::New => {
                ...
                self.record_conversation_items(&items, &turn_context).await;
            }
    ...
    pub(crate) async fn record_conversation_items(
        ...
        self.persist_rollout_response_items(items, turn_context)
            .await;
    }
    ...
    async fn persist_rollout_response_items(
        ...
        self.persist_rollout_items(&rollout_items).await;
        for item in items {
            self.send_event(turn_context, EventMsg::RawItem(item.clone()))
                .await;
        }
    }
    ...
    async fn record_input_and_rollout_usermsg(
        ...
        self.record_conversation_items(std::slice::from_ref(&response_item), turn_context)
            .await;
...
pub(crate) async fn run_task(
    ...
    loop {
        ...
        let turn_input: Vec<ResponseItem> = if is_review_mode {
            ...
            sess.record_conversation_items(&pending_input, &turn_context)
                .await;
